...
class LoginApp:
    ...
    def create_login_widgets(self):
        ...
        self.login_button = tk.Button(self.master, text="登录", command=self.check_login)
        self.login_button.grid(row=2, column=0, padx=5, pady=5, columnspan=2)
    ...
    def check_login(self):
        ...
        if username == "123456" and password == "123456":
            self.create_detection_widgets()
        else:
            tk.messagebox.showerror("错误", "用户名或密码错误！")

    def create_detection_widgets(self):
        ...
        self.stop_button = tk.Button(self.master, text="停止检测", command=self.stop_detection)
        self.stop_button.grid(row=0, column=1, padx=5, pady=5)

    def start_detection(self):
        t = threading.Thread(target=fatigue_detect)
        t.start()

    def stop_detection(self):
        # 在这里停止执行fatigue_detect函数
